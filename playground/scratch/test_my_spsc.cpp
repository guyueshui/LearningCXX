...
TEST_CASE(basic) {
    ...
    // 尝试 push 应该抛异常
    bool thrown = false;
    try { q.push(4); } catch (...) { thrown = true; }
    TEST_ASSERT(thrown);

    // 依次 pop
    TEST_ASSERT(q.front() == 1); q.pop();
    TEST_ASSERT(q.front() == 2); q.pop();
    TEST_ASSERT(q.front() == 3); q.pop();
...
struct Counter {
    ...
    Counter(const Counter& o) : value(o.value) { constructed.fetch_add(1, std::memory_order_relaxed); }
    Counter(Counter&& o) noexcept : value(o.value) { constructed.fetch_add(1, std::memory_order_relaxed); }
