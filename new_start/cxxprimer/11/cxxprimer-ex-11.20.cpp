/******************************************************************************
* File:             cxxprimer-ex-11.20.cpp
*
* Author:           yychi  
* Created:          2019-06-26 21:10 
* Description:      Rewrite your own word count procedure, use "insert" rather
*                   than "operator[]".
*****************************************************************************/
        ...
        WordCnts(map<string, size_t>())
    {
        for (string line; std::getline(infile, line);)
        {
            ...
            for (string text, word; is >> text; word.clear())
            {
                ...
                // pair< map<const string, size_t>::iterator, bool >
                auto iter = WordCnts.insert({word, 1});
                if (! (iter.second) )
                    ++( (iter.first) -> second);
                /** @IDEA
                 *  Using operator[] is more readable and easy. Since it
                 *  handle the above cases automatically.
                 */
